              ...
              CASE 
                WHEN events.duration + events.schedule_date < now() + events.duration   
                     ...
                     events.duration + events.schedule_date = now() + events.duration THEN 
                       CASE events.duration + events.schedule_date < now() THEN 'finished' 
                       ELSE 'live' END
                WHEN events.duration + events.schedule_date > now() + events.duration AND events.is_scheduled = true THEN 'scheduled
           END as "state"
      FROM events 
